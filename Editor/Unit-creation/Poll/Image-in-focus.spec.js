...
test('Big Image', async () => {
    ...
    const versusLayout = await pages[0].waitFor('div[class="layouts-button layouts-item focus-image"]');
    await versusLayout.click();
    await pages[0].type('div[model="slide.title.value"]', 'Image in focus');
