...
test('Big Image', async () => {
    ...
    const bigImageLayout = await pages[0].waitFor('div[class="layouts-button layouts-item big-image"]');
    await bigImageLayout.click();
    await pages[0].type('div[model="slide.title.value"]', 'Big Image');
    let slideAnswers = await pages[0].$$('input[ng-model="option.value"]');
